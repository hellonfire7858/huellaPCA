...
const NV={
  plastico:[{max:30,l:'Eco Héroe',e:'🌿',g:'A+'},{max:55,l:'Consciente',e:'♻️',g:'B'},{max:80,l:'En Desarrollo',e:'⚡',g:'C'},{max:Infinity,l:'En Alerta',e:'🔥',g:'D'}],
  carbono: [{max:2000,l:'Neutro',e:'🌱',g:'A+'},{max:4000,l:'Eficiente',e:'💚',g:'B'},{max:7000,l:'Promedio',e:'⚠️',g:'C'},{max:Infinity,l:'Intensivo',e:'🏭',g:'D'}],
  agua:    [{max:500000,l:'Ahorrador',e:'💎',g:'A+'},{max:900000,l:'Moderado',e:'🌊',g:'B'},{max:1400000,l:'Intenso',e:'⚠️',g:'C'},{max:Infinity,l:'Crítico',e:'🔴',g:'D'}],
};
...
const CC={plastico:['#ff6b35','#ff8c5a','#ffa07a','#ffb59a','#ffc8b5'],carbono:['#39ff14','#5aff3c','#7aff5a','#9aff7a','#baff9a'],agua:['#00d4ff','#22e4ff','#44eefc','#66f5ff','#88faff']};
const TABS=[{k:'plastico',i:'🧴',l:'Plástico'},{k:'carbono',i:'💨',l:'Carbono'},{k:'agua',i:'💧',l:'Agua'}];
...
function Ring({value,max,color,size=124,label,sub}){
  const r=size/2-12,circ=2*Math.PI*r,dash=circ*Math.min(value/max,1);
  return(
...
const TT=({active,payload,label})=>{
  ...
  return <div style={{background:'rgba(8,13,28,0.96)',border:'1px solid rgba(255,255,255,0.09)',borderRadius:10,padding:'9px 14px',backdropFilter:'blur(10px)'}}>
...
function Particles({col}){
  const pts=React.useMemo(()=>Array.from({length:18},(_,i)=>({id:i,x:Math.random()*100,sz:Math.random()*3+1,del:Math.random()*10,dur:Math.random()*12+8,dx:(Math.random()>.5?1:-1)*(Math.random()*40+15)})),[]);
  return<div style={{position:'fixed',inset:0,pointerEvents:'none',zIndex:0,overflow:'hidden'}}>
    {pts.map(p=><div key={p.id} style={{position:'absolute',left:`${p.x}%`,bottom:-8,width:p.sz,height:p.sz,borderRadius:'50%',background:col,opacity:0,boxShadow:`0 0 ${p.sz*3}px ${col}`,'--dx':`${p.dx}px`,animation:`floatUp ${p.dur}s ${p.del}s infinite linear`}}/>)}
  </div>;
}
...
export default function App(){
  ...
  const c=C[tab],Q=AQs[tab],step=steps[tab],a=ans[tab],isDone=done[tab];

  const fp=useMemo(()=>{
    const calc=t=>{const qs=AQs[t],a2=ans[t],cats={};qs.forEach(q=>{cats[q.key]=t==='plastico'?a2[q.key]*q.w:q.calc(a2[q.key]);});return{cats,total:Object.values(cats).reduce((x,y)=>x+y,0)};};
    return{plastico:calc('plastico'),carbono:calc('carbono'),agua:calc('agua')};
  },[ans]);

  const cur=fp[tab],lv=getLv(tab,cur.total);
  const allDone=Object.values(done).every(Boolean);

  const go=next=>{
    setD(next>step?'r':'l');setVis(false);
    setTimeout(()=>{setSteps(s=>({...s,[tab]:next}));setVis(true);},210);
  };
  const setV=(key,val)=>setAns(p=>({...p,[tab]:{...p[tab],[key]:val}}));
  const q=Q[step];

  const liveP=()=>{
    const v=a[q.key],val=tab==='plastico'?v*q.w:q.calc(v);
    return fmt(tab,val);
  };
  ...
  const tips={
    plastico:[{i:'💧',t:'Botella reutilizable',d:'Elimina hasta 70% del plástico en bebidas',k:'botellas',f:0.7},{i:'🛍️',t:'Bolsas de tela',d:'Reemplaza totalmente las bolsas',k:'bolsas',f:0.8},{i:'🥗',t:'Envases reutilizables',d:'Lleva tu contenedor al delivery',k:'envases',f:0.6},{i:'🧼',t:'Productos sólidos',d:'Shampoo en barra = -90%',k:'higiene',f:0.9}],
    carbono: [{i:'🚲',t:'Movilidad activa',d:'1 día en bici/sem reduce ~800kg CO₂',k:'carro',f:0.2},{i:'🌱',t:'Dieta plant-based',d:'Reducir carne 2 días/sem = -30%',k:'carne',f:0.3},{i:'☀️',t:'Energía renovable',d:'Panel solar o tarifa verde = -80%',k:'energia',f:0.8},{i:'✈️',t:'Viaja con propósito',d:'1 vuelo menos = -255 kg CO₂',k:'vuelos',f:0.5}],
    agua:    [{i:'⏱️',t:'Duchas de 5 min',d:'Reduce 40% del consumo en ducha',k:'ducha',f:0.4},{i:'👗',t:'Ciclo corto',d:'Ahorra agua y energía en lavadora',k:'ropa',f:0.25},{i:'🥦',t:'Un día sin carne',d:'Miles de litros ahorrados',k:'dieta',f:0.14},{i:'🚰',t:'Grifo eficiente',d:'Cerrar al cepillarse = 6 L/min',k:'grifo',f:0.8}],
  };

  const diff=cur.total-AVG[tab];

  return(
    <div style={{minHeight:'100vh',background:'#090d1a',color:'#fff',overflowX:'hidden',position:'relative',fontFamily:"system-ui,-apple-system,sans-serif"}}>
      <style>{css}</style>
      <Particles col={c.p}/>
      <div style={{position:'fixed',top:'-18%',left:'-8%',width:'50vw',height:'50vw',borderRadius:'50%',background:`radial-gradient(circle,${c.g} 0%,transparent 68%)`,opacity:.12,pointerEvents:'none',zIndex:0,transition:'background 1s'}}/>
      <div style={{position:'fixed',bottom:'-18%',right:'-8%',width:'42vw',height:'42vw',borderRadius:'50%',background:`radial-gradient(circle,${c.g} 0%,transparent 68%)`,opacity:.09,pointerEvents:'none',zIndex:0,transition:'background 1s'}}/>
      ...
      <div style={{position:'relative',zIndex:1,maxWidth:920,margin:'0 auto',padding:'0 14px 72px'}}>
        ...
        <div style={{textAlign:'center',paddingTop:40,marginBottom:32}}>
          ...
          <h1 style={{fontSize:'clamp(28px,5vw,50px)',fontWeight:900,letterSpacing:'-.03em',lineHeight:1,marginBottom:8,background:`linear-gradient(110deg,#fff 25%,${c.p} 55%,#fff 80%)`,backgroundSize:'200%',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',animation:'shr 4s infinite linear'}}>
            HuellaTotal
          </h1>
          <p style={{color:'rgba(255,255,255,0.35)',fontSize:14,marginBottom:24}}>Mide tu impacto ambiental completo · Colombia 🇨🇴</p>
        ...
        {/* Tabs */}
        <div style={{display:'flex',gap:3,padding:4,borderRadius:16,background:'rgba(255,255,255,0.025)',border:'1px solid rgba(255,255,255,0.055)',marginBottom:24}}>
          {TABS.map(t=>{const tc=C[t.k],act=tab===t.k;return(
            <button key={t.k} onClick={()=>setTab(t.k)} style={{flex:1,padding:'10px 4px',borderRadius:12,border:'none',cursor:'pointer',fontSize:'clamp(11px,2.5vw,14px)',fontWeight:700,fontFamily:'inherit',transition:'all .3s',background:act?`${tc.p}1c`:'transparent',color:act?tc.p:'rgba(255,255,255,0.28)',boxShadow:act?`0 0 16px ${tc.g}44`:'none',position:'relative'}}>
              {t.i} {t.l}{done[t.k]&&<sup style={{fontSize:8,color:tc.t,marginLeft:3}}>✓</sup>}
          ...
          <div style={{maxWidth:520,margin:'0 auto'}}>
            ...
            <div style={{marginBottom:18}}>
              <div style={{display:'flex',justifyContent:'space-between',marginBottom:7}}>
                <span style={{fontSize:12,color:'rgba(255,255,255,0.4)',fontWeight:600}}>Pregunta {step+1}/{Q.length}</span>
                <span style={{fontSize:12,fontWeight:800,color:c.p}}>{Math.round((step+1)/Q.length*100)}%</span>
              </div>
              <div style={{display:'flex',gap:4}}>
                {Q.map((_,i)=><div key={i} style={{flex:1,height:4,borderRadius:2,transition:'all .4s',background:i<=step?c.p:'rgba(255,255,255,0.07)',boxShadow:i<=step?`0 0 7px ${c.g}44`:'none'}}/>)}
              </div>
            </div>
            ...
            <div style={{borderRadius:20,padding:'28px 24px',marginBottom:16,opacity:vis?1:0,transform:vis?'translateX(0)':`translateX(${d==='r'?'20px':'-20px'})`,transition:'all .21s ease',background:'rgba(255,255,255,0.035)',backdropFilter:'blur(20px)',border:'1px solid rgba(255,255,255,0.08)',boxShadow:'0 8px 44px rgba(0,0,0,0.44),inset 0 1px 0 rgba(255,255,255,0.06)'}}>
              ...
              <div style={{fontSize:12,color:'rgba(255,255,255,0.32)',marginBottom:24}}>{q.hint}</div>
              <div style={{display:'flex',alignItems:'baseline',gap:7,marginBottom:7}}>
                ...
                <span style={{fontSize:12,color:'rgba(255,255,255,0.32)'}}>{q.unit}</span>
              </div>
              <div style={{'--tc':c.p,'--tg':c.g}}>
                <input type="range" min="0" max={q.max} value={a[q.key]} onChange={e=>setV(q.key,+e.target.value)} style={{width:'100%',marginBottom:3}}/>
              </div>
              <div style={{display:'flex',justifyContent:'space-between',fontSize:10,color:'rgba(255,255,255,0.18)',marginBottom:18}}><span>0</span><span>{q.max}</span></div>
            ...
            <div style={{display:'flex',gap:9}}>
              {step>0&&<button onClick={()=>go(step-1)} style={{flex:1,padding:'12px',borderRadius:13,border:'1px solid rgba(255,255,255,0.09)',background:'rgba(255,255,255,0.04)',color:'rgba(255,255,255,0.6)',fontSize:13,fontWeight:600,cursor:'pointer',backdropFilter:'blur(8px)',fontFamily:'inherit'}}>← Atrás</button>}
              <button onClick={()=>{if(step<Q.length-1)go(step+1);else setDone(dd=>({...dd,[tab]:true}));}} style={{flex:2,padding:'12px',borderRadius:13,border:'none',background:`linear-gradient(135deg,${c.p},${c.s})`,color:'#040810',fontSize:14,fontWeight:800,cursor:'pointer',boxShadow:`0 4px 20px ${c.g}`,fontFamily:'inherit'}}>
                {step<Q.length-1?'Siguiente →':`Ver mi huella ${q.emoji}`}
              </button>
            </div>
          </div>
        ):(
          <div style={{animation:'scIn .5s ease'}}>
            {/* Hero */}
            <div style={{textAlign:'center',marginBottom:28}}>
              <div style={{display:'inline-block',marginBottom:18}}>
                <div style={{borderRadius:26,padding:'32px 48px',background:`radial-gradient(ellipse at 30% 20%,${c.p}18 0%,rgba(0,0,0,0) 65%)`,border:`1px solid ${c.p}28`,boxShadow:`0 0 55px ${c.g}44,inset 0 1px 0 ${c.p}15`,backdropFilter:'blur(20px)',position:'relative',overflow:'hidden'}}>
                  ...
                  <div style={{fontSize:'clamp(52px,9vw,84px)',fontWeight:900,color:c.p,lineHeight:1,textShadow:`0 0 36px ${c.g},0 0 72px ${c.g}44`}}>{fmt(tab,cur.total).split(' ')[0]}</div>
                  <div style={{fontSize:18,color:'rgba(255,255,255,0.45)',marginBottom:12}}>{fmt(tab,cur.total).replace(/^[\d.,]+k?\s*/,'')} al año</div>
                  <div style={{display:'inline-block',padding:'7px 20px',borderRadius:40,background:`${c.p}1e`,border:`1px solid ${c.p}48`,color:c.p,fontWeight:800,fontSize:14,boxShadow:`0 0 16px ${c.g}55`}}>{lv.l} · Grado {lv.g}</div>
                </div>
              </div>
              <div style={{display:'flex',gap:12,justifyContent:'center',flexWrap:'wrap'}}>
                <div className="glass" style={{borderRadius:13,padding:'13px 20px',textAlign:'center'}}>
                  <div style={{fontSize:18,fontWeight:800,color:diff<0?'#39ff14':'#ff6b35'}}>{diff<0?'▼':'▲'} {fmt(tab,Math.abs(diff))}</div>
                  <div style={{fontSize:10,color:'rgba(255,255,255,0.3)',marginTop:3}}>{diff<0?'menos':'más'} que el promedio</div>
                </div>
                <div className="glass" style={{borderRadius:13,padding:'13px 20px',textAlign:'center'}}>
                  <div style={{fontSize:18,fontWeight:800,color:'rgba(255,255,255,0.65)'}}>{fmt(tab,AVG[tab])}</div>
                  <div style={{fontSize:10,color:'rgba(255,255,255,0.3)',marginTop:3}}>promedio Colombia</div>
                </div>
              </div>
            </div>
              ...
              <div className="glass" style={{borderRadius:18,padding:20}}>
                ...
                <ResponsiveContainer width="100%" height={180}>
                  <BarChart data={bar} barSize={26}>
                    ...
                    <XAxis dataKey="name" tick={{fill:'rgba(255,255,255,0.38)',fontSize:13}}/>
                    <YAxis tick={{fill:'rgba(255,255,255,0.22)',fontSize:9}}/>
                    <Tooltip content={<TT/>}/>
                    <Bar dataKey="v" radius={[7,7,0,0]}>{bar.map((_,i)=><Cell key={i} fill={CC[tab][i]}/>)}</Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
              <div className="glass" style={{borderRadius:18,padding:20}}>
                ...
                {pie.length>0?<ResponsiveContainer width="100%" height={180}>
                  <PieChart>
                    <Pie data={pie} cx="50%" cy="50%" outerRadius={68} innerRadius={30} dataKey="value" label={({name,percent})=>percent>.07?`${name} ${(percent*100).toFixed(0)}%`:''} labelLine={false} fontSize={11}>
                      {pie.map((_,i)=><Cell key={i} fill={CC[tab][i]}/>)}
                    </Pie>
                    <Tooltip content={<TT/>}/>
                  </PieChart>
                </ResponsiveContainer>:<div style={{height:180,display:'flex',alignItems:'center',justifyContent:'center',color:'rgba(255,255,255,0.22)',fontSize:13}}>Mueve los sliders para ver datos</div>}
              </div>
            </div>
                  ...
                  {Q.map(q2=>(
                    <div key={q2.key} style={{marginBottom:10}}>
                      <div style={{display:'flex',justifyContent:'space-between',marginBottom:3}}>
                        <span style={{fontSize:11,color:'rgba(255,255,255,0.45)'}}>{q2.emoji} {q2.key}</span>
                        <span style={{fontSize:11,fontWeight:700,color:c.p}}>{fmt(tab,cur.cats[q2.key])}</span>
                      </div>
                      <div style={{height:4,borderRadius:2,background:'rgba(255,255,255,0.06)'}}>
                        <div style={{height:'100%',borderRadius:2,background:`linear-gradient(90deg,${c.p},${c.s})`,width:`${cur.total>0?Math.min(cur.cats[q2.key]/cur.total*100,100):0}%`,boxShadow:`0 0 7px ${c.g}`,transition:'width 1s cubic-bezier(.4,0,.2,1)'}}/>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(190px,1fr))',gap:10}}>
                {tips[tab].map((t,i)=>(
                  <div key={i} className="tc" style={{borderRadius:14,padding:'14px 16px',background:c.bg,border:`1px solid ${c.p}20`,cursor:'default'}}>
                    ...
                    <div style={{fontSize:11,color:'rgba(255,255,255,0.38)',marginBottom:8,lineHeight:1.4}}>{t.d}</div>
            ...
            {/* Radar */}
            {allDone&&(
              <div className="glass" style={{borderRadius:18,padding:24,marginBottom:16,textAlign:'center'}}>
                <h3 style={{fontSize:14,fontWeight:700,marginBottom:4}}>🌍 Perfil Ambiental Completo</h3>
                <p style={{fontSize:11,color:'rgba(255,255,255,0.28)',marginBottom:14}}>% respecto al doble del promedio colombiano</p>
                <ResponsiveContainer width="100%" height={230}>
                  <RadarChart data={radar}>
                    <PolarGrid stroke="rgba(255,255,255,0.07)"/>
                    <PolarAngleAxis dataKey="s" tick={{fill:'rgba(255,255,255,0.5)',fontSize:12,fontWeight:600}}/>
                    <Radar dataKey="A" stroke="#fff" fill="rgba(255,255,255,0.07)" fillOpacity={0.9} dot={{fill:'#fff',r:4,strokeWidth:0}}/>
                  </RadarChart>
                </ResponsiveContainer>
                <button onClick={()=>{const txt=`🌎 Mi Huella Ambiental:\n🧴 Plástico: ${fmt('plastico',fp.plastico.total)}\n💨 Carbono: ${fmt('carbono',fp.carbono.total)}\n💧 Agua: ${fmt('agua',fp.agua.total)}\n— HuellaTotal 🌿`;navigator.clipboard.writeText(txt).then(()=>{setCop(true);setTimeout(()=>setCop(false),2500);});}} style={{padding:'11px 30px',borderRadius:40,border:'none',background:'linear-gradient(135deg,#ff6b35,#39ff14,#00d4ff)',backgroundSize:'200%',color:'#040810',fontWeight:800,fontSize:14,cursor:'pointer',boxShadow:'0 4px 24px rgba(0,0,0,0.38)',marginTop:12,fontFamily:'inherit'}}>
            ...
            <div style={{borderRadius:20,padding:'26px 22px',textAlign:'center',background:`linear-gradient(135deg,${c.p}18,${c.s}0c)`,border:`1px solid ${c.p}25`,boxShadow:`0 0 36px ${c.g}1e`,marginBottom:16}}>
              ...
              <p style={{color:'rgba(255,255,255,0.38)',marginBottom:20,fontSize:13}}>
                {!allDone?`Calcula también tu huella de ${tab==='plastico'?'carbono y agua':tab==='carbono'?'plástico y agua':'plástico y carbono'}.`:'¡Tienes tu perfil ambiental completo!'}
              </p>
              <div style={{display:'flex',gap:9,justifyContent:'center',flexWrap:'wrap'}}>
                <button onClick={()=>{setDone(dd=>({...dd,[tab]:false}));setSteps(ss=>({...ss,[tab]:0}));}} style={{padding:'10px 20px',borderRadius:13,border:'1px solid rgba(255,255,255,0.09)',background:'rgba(255,255,255,0.04)',color:'rgba(255,255,255,0.6)',fontSize:13,fontWeight:600,cursor:'pointer',backdropFilter:'blur(8px)',fontFamily:'inherit'}}>↺ Recalcular</button>
                {TABS.filter(t=>!done[t.k]&&t.k!==tab).map(t=>(
                  <button key={t.k} onClick={()=>setTab(t.k)} style={{padding:'10px 20px',borderRadius:13,border:'none',background:`linear-gradient(135deg,${C[t.k].p},${C[t.k].s})`,color:'#040810',fontWeight:800,fontSize:13,cursor:'pointer',boxShadow:`0 4px 16px ${C[t.k].g}`,fontFamily:'inherit'}}>{t.i} {t.l}</button>
                ))}
                {allDone&&<button onClick={()=>{const txt=`🌎 Mi Huella Ambiental:\n🧴 Plástico: ${fmt('plastico',fp.plastico.total)}\n💨 Carbono: ${fmt('carbono',fp.carbono.total)}\n💧 Agua: ${fmt('agua',fp.agua.total)}\n— HuellaTotal 🌿`;navigator.clipboard.writeText(txt).then(()=>{setCop(true);setTimeout(()=>setCop(false),2500);});}} style={{padding:'10px 20px',borderRadius:13,border:'none',background:`linear-gradient(135deg,${c.p},${c.s})`,color:'#040810',fontWeight:800,fontSize:13,cursor:'pointer',fontFamily:'inherit'}}>{cop?'✅ ¡Copiado!':'📤 Compartir'}</button>}
              </div>
            </div>

            <div style={{textAlign:'center',color:'rgba(255,255,255,0.15)',fontSize:11,paddingBottom:8}}>Datos basados en promedios Colombia · Cada acción cuenta 🌍</div>
          </div>
        )}
      </div>
    </div>
